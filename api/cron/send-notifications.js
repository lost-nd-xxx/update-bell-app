...
export default async function handler(request, response) {
  ...
  try {
    ...
    for (const userId in remindersByUser) {
      ...
      // 処理済みのリマインダーをKVとSorted Setから削除
      const deleteTx = kv.multi();
      const reminderKeysToDelete = userReminders.map((rem) => rem.key);
      deleteTx.del(...reminderKeysToDelete);
      deleteTx.zrem(sortedSetKey, ...reminderKeysToDelete);
      await deleteTx.exec();
      console.log(
        `[CRON] Deleted ${reminderKeysToDelete.length} processed reminders for user ${userId}.`,
      );
    }
