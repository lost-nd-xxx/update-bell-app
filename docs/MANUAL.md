...
- **PC (Chromeなど)**: アドレスバー右端に表示される「インストール」アイコンをタップします。
...
1.  **「+」ボタンをタップ**: アプリのダッシュボード画面にある「+」ボタンをタップします。
...
- **毎週○曜日**: 曜日（例: 毎週月曜と水曜）を選択して通知します。
- **○週間ごと**: 特定の曜日（例: 毎週月曜日）に通知します。
- **毎月第○週**: 月の第n週の特定曜日（例: 毎月第2週の火曜日）に通知します。
...
- **プッシュ通知の設定**: プッシュ通知を有効にすることができます。リマインダーを作成・編集するには、プッシュ通知の購読が必須です。
- **データ管理**:
  - **データエクスポート**: 設定したリマインダーやアプリの設定をJSONファイルとしてダウンロードし、バックアップできます。エクスポートされるデータには、リマインダーの設定内容（タイトル、URL、スケジュール、タグなど）とアプリの設定（テーマなど）が含まれます。システム内部のID、通知履歴、実行ステータスなどは含まれません。
  - **データインポート**: バックアップしたJSONファイルからデータを復元できます。インポート時には以下の点にご注意ください:
    - プッシュ通知の購読が必要です（未購読の場合はエラーになります）
    - オンライン接続が必要です
    - 既存のリマインダーと重複する内容（タイトル、URL、スケジュールがすべて一致）は自動的にスキップされます
    - インポートされたリマインダーには新しいIDが割り当てられ、通知履歴はリセットされます
...
- **サーバーデータ**: リマインダーの内容と通知先情報がサーバー（Vercel KV）に保存されます。
- **データ自動削除**: 最後に通知をタップしてから半年間ご利用がないユーザーのサーバーデータ（リマインダーおよび購読情報）は、自動的に削除されます。詳細は[プライバシーポリシー](PRIVACY_POLICY.md)をご確認ください。
...
一部の機能はオフラインで利用できますが、リマインダーの作成・編集・削除やデータのインポートには通信が必要です。
...
- **オフラインでできないこと（オンライン接続が必要です）:**
  - リマインダーの新規作成・編集・削除
  - プッシュ通知の有効化・無効化
  - データのインポート（ローカルとサーバーのデータ整合性を保つため）

オフライン状態でこれらの操作を行おうとすると、エラーメッセージが表示されます。その場合は、オンライン環境で再度操作をお試しください。
