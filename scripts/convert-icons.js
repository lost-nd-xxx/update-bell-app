...
const generateMaskableIcon = async (size) => {
  ...
  try {
    log(
      `🔄 生成中: マスキング可能アイコン ${size}x${size}px (元: ${path.basename(svgPath)})`,
    );
    ...
    log(`✅ 完了: icon-maskable-${size}x${size}.png (${fileSizeKB}KB)`);
  } catch (error) {
    console.error(
      `❌ エラー: マスキング可能アイコン ${size}x${size}px 生成失敗`,
    );
    console.error(error.message);
    throw error;
  }
...
const main = async () => {
  try {
    ...
    try {
      ...
      log("✅ 完了: icon-badge.png");
    } catch (error) {
      console.error("❌ エラー: icon-badge.png のコピー失敗");
      console.error(error.message);
      // エラーが発生しても処理は続行
    }

    log("=== 🎉 アイコン変換完了！ ===");
    log(`📊 生成ファイル数: ${iconSizes.length + 3}個`);
    log(
      "📁 出力先: public/icon-*, public/icon-maskable-*.png, public/favicon.png",
    );
  } catch (error) {
    console.error("\n❌ アイコン変換でエラーが発生しました:");
    console.error(error.message);
